...
```{r}
summary(semTools::compareFit(fitA, fitB))
```
...
```{r}
summary(semTools::compareFit(fitA_rob, fitB_rob))
```

To use the robust variant of $\chi^2$, we set `robust = TRUE`:
```{r}
tosdif(fitA_rob, fitB_rob, robust = TRUE)
```

$p$ is now $=.912$, so the interpretation does not change in this example.